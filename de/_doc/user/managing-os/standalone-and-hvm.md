...
A [StandaloneVM](/de/doc/glossary/#standalonevm) is a type of VM in Qubes that is created by cloning a [TemplateVM](/de/doc/templates/).
...
Meanwhile, a [Hardware-assisted Virtual Machine (HVM)](/de/doc/glossary/#hvm), also known as a "Fully-Virtualized Virtual Machine," utilizes the virtualization extensions of the host CPU.
These are typically contrasted with [Paravirtualized (PV)](/de/doc/glossary/#pv) VMs.
...
By default, every Qubes VM runs in [PVH](/de/doc/glossary/#pvhvm) mode (which has security advantages over both PV and HVM) except for those with attached PCI devices, which run in HVM mode.
...
In fact, this is so common that [StandaloneHVMs](/de/doc/glossary/#standalonehvm) are typically just called "HVMs."
...
Normally Qubes agent scripts (or services on Windows) running within each AppVM are responsible for setting up networking within the VM according to the configuration created by Qubes (through [keys](/de/doc/vm-interface/#qubesdb) exposed by dom0 to the VM). 
...
There is [opt-in support](/de/doc/networking/#ipv6) for IPv6 forwarding.


## Using TemplateBasedHVMs

Qubes allows HVMs to share a common root filesystem from a select TemplateVM (see [TemplateHVM](/de/doc/glossary/#templatehvm) and [TemplateBasedHVM](/de/doc/glossary/#templatebasedhvm)).
...
... , set memory as appropriate, and install the OS into this template in the same way you would install it into a normal HVM -- please see instructions on [this page](/de/doc/hvm-create/).
...
Please see [this page](/de/doc/windows-appvms/) for specific advice on installing and using Windows-based Templates.
...
HVM domains (including Windows VMs) can be [assigned PCI devices](/de/doc/assigning-devices/) just like normal AppVMs.
