---
lang: de
layout: doc-index
permalink: /de/doc/
redirect_from:
- /de/doc/UserDoc/
- /de/wiki/UserDoc/
- /de/doc/QubesDocs/
- /de/wiki/QubesDocs/
- /de/help/
- /de/help/
- /de/community/
- /de/community/
ref: 1
title: Documentation
translated: 'yes'
---

## Introduction

 * [What is Qubes OS?](/de/intro/)
 * [Video Tours](/de/video-tours/)
 * [Screenshots](/de/screenshots/)
 * [User FAQ](/de/faq/#users)
 * [Reporting Bugs and Other Issues](/de/doc/reporting-bugs/)
 * [Help, Support, and Mailing Lists](/de/support/)
 * [How to Contribute](/de/doc/contributing/)

## Project Security

 * [Security Center](/de/security/)
 * [Security FAQ](/de/faq/#general--security)
 * [Security Pack](/de/security/pack/)
 * [Security Bulletins](/de/security/bulletins/)
 * [Canaries](/de/security/canaries/)
 * [Xen Security Advisory (XSA) Tracker](/de/security/xsa/)
 * [Verifying Signatures](/de/security/verifying-signatures/)
 * [Qubes PGP Keys](https://keys.qubes-os.org/keys/)


## User Documentation

Core documentation for Qubes users.

### Choosing Your Hardware

 * [System Requirements](/de/doc/system-requirements/)
 * [Certified Hardware](/de/doc/certified-hardware/)
 * [Hardware Compatibility List (HCL)](/de/hcl/)
 * [Hardware Testing](/de/doc/hardware-testing/)

### Downloading, Installing, and Upgrading Qubes

 * [Downloads](/de/downloads/)
 * [Installation Guide](/de/doc/installation-guide/)
 * [Upgrade Guides](/de/doc/upgrade/)
 * [Supported Versions](/de/doc/supported-versions/)
 * [Version Scheme](/de/doc/version-scheme/)
 * [Testing New Releases and Updates](/de/doc/testing/)

### Common Tasks

 * [Getting Started](/de/getting-started/)
 * [Copying and Pasting Text Between Domains](/de/doc/copy-paste/)
 * [Copying and Moving Files Between Domains](/de/doc/copying-files/)
 * [Copying from (and to) Dom0](/de/doc/copy-from-dom0/)
 * [Updating Qubes OS](/de/doc/updating-qubes-os/)
 * [Installing and Updating Software in Dom0](/de/doc/software-update-dom0/)
 * [Installing and Updating Software in DomUs](/de/doc/software-update-domu/)
 * [Backup, Restoration, and Migration](/de/doc/backup-restore/)
 * [DisposableVMs](/de/doc/disposablevm/)
 * [Block (or Storage) Devices](/de/doc/block-devices/)
 * [USB Devices](/de/doc/usb-devices)
 * [PCI Devices](/de/doc/pci-devices/)
 * [Device Handling](/de/doc/device-handling/)
 * [Optical Discs](/de/doc/optical-discs/)
 * [Application Shortcuts](/de/doc/managing-appvm-shortcuts/)
 * [Fullscreen Mode](/de/doc/full-screen-mode/)

### Managing Operating Systems within Qubes

 * [TemplateVMs](/de/doc/templates/)
 * [Fedora](/de/doc/templates/fedora/)
 * [Debian](/de/doc/templates/debian/)
 * [Minimal TemplateVMs](/de/doc/templates/minimal/)
 * [Windows](/de/doc/windows/)
 * [StandaloneVMs and HVMs](/de/doc/standalone-and-hvm/)

### Security in Qubes

 * [Qubes Firewall](/de/doc/firewall/)
 * [Understanding and Preventing Data Leaks](/de/doc/data-leaks/)
 * [Passwordless Root Access in VMs](/de/doc/vm-sudo/)
 * [Device Handling Security](/de/doc/device-handling-security/)
 * [Anti Evil Maid](/de/doc/anti-evil-maid/)
 * [Split GPG](/de/doc/split-gpg/)
 * [U2F Proxy](/de/doc/u2f-proxy/)
 * [YubiKey](/de/doc/yubi-key/)

### Advanced Configuration

 * [Configuration Files](/de/doc/config-files/)
 * [Storing AppVMs on Secondary Drives](/de/doc/secondary-storage/)
 * [RPC Policies](/de/doc/rpc-policy/)
 * [USB Qubes](/de/doc/usb-qubes/)
 * [Managing VM Kernels](/de/doc/managing-vm-kernel/)
 * [Salt Management Stack](/de/doc/salt/)
 * [DisposableVM Customization](/de/doc/disposablevm-customization/)
 * [Making Any File Persistent Using `bind-dirs`](/de/doc/bind-dirs/)
 * [GUI Configuration](/de/doc/gui-configuration/)
 * [Resizing Disk Images](/de/doc/resize-disk-image/)
 * [Troubleshooting UEFI](/de/doc/uefi-troubleshooting/)
 * [Troubleshooting Newer Hardware](/de/doc/newer-hardware-troubleshooting/)
 * [Mounting and Decrypting Qubes Partitions from Outside Qubes](/de/doc/mount-from-other-os/)
 * [KDE](/de/doc/kde/)
 * [i3 Window Manager](/de/doc/i3/)
 * [awesome Window Manager](/de/doc/awesome/)

### Reference Pages

 * [Command-line Tools](/de/doc/tools/)
 * [Glossary](/de/doc/glossary/)
 * [Qubes Service Framework](/de/doc/qubes-service/)
 * [Command Execution in VMs (and Qubes RPC)](/de/doc/qrexec/)
 * [Deprecated Documentation](https://github.com/QubesOS/qubesos.github.io#deprecated-documentation)


## Developer Documentation

Core documentation for Qubes developers and advanced users.

### General

 * [Developer FAQ](/de/faq/#developers)
 * [Package Contributions](/de/doc/package-contributions/)
 * [Documentation Guidelines](/de/doc/doc-guidelines/)
 * [Community-Developed Feature Tracker](/qubes-issues/)
 * [Google Summer of Code](/de/gsoc/)
 * [Google Season of Docs](/de/gsod/)
 * [Books for Developers](/de/doc/devel-books/)
 * [Style Guide](/de/doc/style-guide/)
 * [Usability & UX](/de/doc/usability-ux/)

### Code

 * [Source Code](/de/doc/source-code/)
 * [Software License](/de/doc/license/)
 * [Coding Guidelines](/de/doc/coding-style/)
 * [Code Signing](/de/doc/code-signing/)

### System

 * [Qubes OS Architecture Overview](/de/doc/architecture/)
 * [Qubes OS Architecture Spec v0.3 [PDF]](/attachment/wiki/QubesArchitecture/arch-spec-0.3.pdf)
 * [Security-critical Code in Qubes OS](/de/doc/security-critical-code/)
 * [Qubes Core Admin](https://dev.qubes-os.org/projects/core-admin/en/latest/)
 * [Qubes Core Admin Client](https://dev.qubes-os.org/projects/core-admin-client/en/latest/)
 * [Qubes Admin API](/news/2017/06/27/qubes-admin-api/)
 * [Qubes Core Stack](/news/2017/10/03/core3/)
 * [Qubes GUI virtualization protocol](/de/doc/gui/)
 * [Networking in Qubes](/de/doc/networking/)
 * [Implementation of template sharing and updating](/de/doc/template-implementation/)
 * [Storage Pools](/de/doc/storage-pools/)
 * [Audio virtualization](/de/doc/audio-virtualization/)

### Services

 * [Inter-domain file copying](/de/doc/qfilecopy/) (deprecates [`qfileexchgd`](/de/doc/qfileexchgd/))
 * [Dynamic memory management in Qubes](/de/doc/qmemman/)
 * [Implementation of DisposableVMs](/de/doc/dvm-impl/)
 * [Dom0 secure update mechanism](/de/doc/dom0-secure-updates/)
 * [Qrexec: secure communication across domains](/de/doc/qrexec/)
 * [Qrexec: Qubes RPC internals](/de/doc/qrexec-internals/)

### Debugging

 * [Profiling python code](/de/doc/profiling/)
 * [Test environment in separate machine for automatic tests](/de/doc/test-bench/)
 * [Automated tests](/de/doc/automated-tests/)
 * [VM-dom0 internal configuration interface](/de/doc/vm-interface/)
 * [Debugging Windows VMs](/de/doc/windows-debugging/)
 * [Safe Remote Dom0 Terminals](/de/doc/safe-remote-ttys/)
 * [Mount LVM Image](/de/doc/mount-lvm-image/)

### Building

 * [Building Qubes](/de/doc/qubes-builder/) (["API" Details](/de/doc/qubes-builder-details/))
 * [Development Workflow](/de/doc/development-workflow/)
 * [Building Qubes OS ISO](/de/doc/qubes-iso-building/)
 * [Qubes Template Configuration Files](https://github.com/QubesOS/qubes-template-configs)

### Releases

 * [Release notes](/de/doc/releases/notes/)
 * [Release schedules](/de/doc/releases/schedules/)
 * [Release checklist](/de/doc/releases/todo/)


## External Documentation

Unofficial, third-party documentation from the Qubes community and others.

 * [Qubes Community Documentation](https://github.com/Qubes-Community/Contents/tree/master/docs)

### Operating System Guides

 * [Template: Ubuntu](/de/doc/templates/ubuntu/)
 * [Template: Whonix](/de/doc/whonix/)
 * [Pentesting](/de/doc/pentesting/)
 * [Pentesting: BlackArch](/de/doc/pentesting/blackarch/)
 * [Pentesting: Kali](/de/doc/pentesting/kali/)
 * [Pentesting: PTF](/de/doc/pentesting/ptf/)
 * [Tips for Using Linux in an HVM](/de/doc/linux-hvm-tips/)
 * [Creating a NetBSD VM](/de/doc/netbsd/)

### Security Guides

 * [Security Guidelines](/de/doc/security-guidelines/)
 * [Using Multi-factor Authentication with Qubes](/de/doc/multifactor-authentication/)
 * [How to Set Up a Split Bitcoin Wallet in Qubes](/de/doc/split-bitcoin/)
 * [Split dm-crypt](https://github.com/rustybird/qubes-split-dm-crypt)
 * [Split SSH](https://kushaldas.in/posts/using-split-ssh-in-qubesos-4-0.html)
 * [Using OnlyKey with Qubes OS](https://docs.crp.to/qubes.html)

### Privacy Guides

 * [Whonix for Privacy & Anonymity](/de/doc/whonix/)
 * [Running Tails in Qubes](/de/doc/tails/)
 * [Anonymizing your MAC Address](/de/doc/anonymizing-your-mac-address/)
 * [Martus](/de/doc/martus/)
 * [Signal](/de/doc/signal/)
 * [Reducing the fingerprint of the text-based web browser w3m](/de/doc/w3m/)

### Configuration Guides

 * [Qubes Tips and Tricks](/de/doc/tips-and-tricks/)
 * [How to set up a ProxyVM as a VPN Gateway](/de/doc/vpn/)
 * [Multibooting](/de/doc/multiboot/)
 * [Changing your Time Zone](/de/doc/change-time-zone/)
 * [Installing ZFS in Qubes](/de/doc/zfs/)
 * [Mutt Guide](/de/doc/mutt/)
 * [Postfix Guide](/de/doc/postfix/)
 * [Fetchmail Guide](/de/doc/fetchmail/)
 * [Creating Custom NetVMs and ProxyVMs](https://theinvisiblethings.blogspot.com/2011/09/playing-with-qubes-networking-for-fun.html)
 * [How to make proxy for individual tcp connection from networkless VM](https://groups.google.com/group/qubes-devel/msg/4ca950ab6d7cd11a)
 * [Adding Bridge Support to the NetVM (EXPERIMENTAL)](/de/doc/network-bridge-support/)
 * [Enabling TRIM for SSD disks](/de/doc/disk-trim/)
 * [Configuring a Network Printer](/de/doc/network-printer/)
 * [Using External Audio Devices](/de/doc/external-audio/)
 * [Rxvt Guide](/de/doc/rxvt/)
 * [Adding SSD storage cache](https://groups.google.com/d/msgid/qubes-users/a08359c9-9eb0-4d1a-ad92-a8a9bc676ea6%40googlegroups.com)
 * [How to Make a Multimedia TemplateVM](/de/doc/multimedia/)

### Customization Guides

 * [Customizing Fedora minimal templates](/de/doc/fedora-minimal-template-customization/)
 * [Customizing Windows 7 templates](/de/doc/windows-template-customization/)
 * [Language Localization](/de/doc/language-localization/)
 * [Dark Theme in Dom0 and DomU](/de/doc/dark-theme/)
 * [Safely Removing TemplateVM Packages (Example: Thunderbird)](/de/doc/removing-templatevm-packages/)

### Troubleshooting

 * [Home directory is out of disk space error](/de/doc/out-of-memory/)
 * [Installing on system with new AMD GPU (missing firmware problem)](https://groups.google.com/group/qubes-devel/browse_thread/thread/e27a57b0eda62f76)
 * [How to install an Nvidia driver in dom0](/de/doc/install-nvidia-driver/)
 * [Nvidia troubleshooting guide](/de/doc/nvidia-troubleshooting/)
 * [Lenovo ThinkPad Troubleshooting](/de/doc/thinkpad-troubleshooting/)
 * [Apple MacBook Troubleshooting](/de/doc/macbook-troubleshooting/)
 * [Getting Sony Vaio Z laptop to work with Qubes](/de/doc/sony-vaio-tinkering/)
 * [Fixing wireless on suspend & resume](/de/doc/wireless-troubleshooting/)
 * [How to remove VMs manually](/de/doc/remove-vm-manually/)
 * [Intel Integrated Graphics Troubleshooting](/de/doc/intel-igfx-troubleshooting/)

### Building Guides

 * [Building a TemplateVM based on a new OS (ArchLinux example)](/de/doc/building-non-fedora-template/)
 * [Building the Archlinux Template](/de/doc/building-archlinux-template/)
 * [Building the Whonix Templates](/de/doc/building-whonix-template/)
 * [How to compile kernels for dom0](https://groups.google.com/d/topic/qubes-users/yBeUJPwKwHM/discussion)